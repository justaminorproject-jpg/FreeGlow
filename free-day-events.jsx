...
export default function App() {
  ...
  const doSearch = async () => {
    if(!keyOk){setError("API key not configured. Set VITE_GROQ_API_KEY in your environment.");return;}
...
Provide 2-4 events per date. Make them realistic and specific to the area.`;
    ...
    catch(e){ setError("Couldn't load events. ("+e.message+")"); }
    setLoadingEvents(false);
  };

  // ── Search meetups ─────────────────────────────────────────────────────────
  const doMeetups = async () => {
    if(!keyOk){setMeetupError("API key not configured. Set VITE_GROQ_API_KEY in your environment.");return;}
...
Provide 6-9 diverse groups across different interests. Be specific to the city and age group.`;
    ...
    catch(e){ setMeetupError("Couldn't load meetups. ("+e.message+")"); }
